...
with strategy.scope():
    ...
    drop = 0.5
    encoded = Dense(1300, activation='tanh')(input_prot)
    encoded = Dropout(drop)(encoded)
    encoded = Dense(1000, activation='tanh')(encoded)
    encoded = Dropout(drop)(encoded)
    encoded = Dense(600, activation='tanh')(encoded)
    encoded = Dropout(drop)(encoded)
    ...
    decoded = Dropout(drop)(decoded)
    decoded = Dense(600, activation='tanh')(decoded)
    decoded = Dropout(drop)(decoded)
    decoded = Dense(100, activation='tanh')(decoded)
    decoded = Dropout(drop)(decoded)
    decoded = Dense(1300, activation='tanh')(decoded)
    decoded = Dropout(drop)(decoded)
...
plt.scatter(xG,yG,zG, color='r',label='K50E mut',alpha=0.5)
plt.scatter(xW,yW,zW, color='b',label='WT',alpha=0.5)
plt.scatter(xS,yS,zS, color='black', marker='X', label="Starting Struct")
