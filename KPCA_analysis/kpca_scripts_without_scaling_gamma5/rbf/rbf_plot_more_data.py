...
kernel_pca_rbf = KernelPCA(
    n_components=None, kernel="rbf", fit_inverse_transform=True, alpha=0.1, gamma=5
)
# rbf_kernel_pca = kernel_pca_rbf.fit(train_data).transform(test_data)

rbf_kernel_pca = kernel_pca_rbf.fit(train_data)
rbf_train_kernel_pca = rbf_kernel_pca.transform(train_data)
...
kernel_pca_proj_ax.scatter(rbf_train_kernel_pca[:, 0], rbf_train_kernel_pca[:, 1])
kernel_pca_proj_ax.set_ylabel("Principal component #1")
kernel_pca_proj_ax.set_xlabel("Principal component #0")
_ = kernel_pca_proj_ax.set_title("Projection of training data\n using KernelPCA")

fig.savefig('TrainingData2.png', bbox_inches = "tight", dpi= 2000)
...
rbf_GLU_kernel_pca_normed = rbf_kernel_pca.transform(GLU)
rbf_WT_kernel_pca_normed = rbf_kernel_pca.transform(WT)
rbf_ST_kernel_pca_normed = rbf_kernel_pca.transform(ST)

plt.figure(figsize=(12, 8))
plt.scatter(rbf_GLU_kernel_pca_normed[:, 0], rbf_GLU_kernel_pca_normed[:, 1])
plt.ylabel("Feature #1")
plt.xlabel("Feature #0")
plt.title("GLU data")
plt.savefig('GLUData_more_data.png', bbox_inches = "tight", dpi= 2000)


plt.figure(figsize=(12, 8))
plt.scatter(rbf_WT_kernel_pca_normed[:, 0], rbf_WT_kernel_pca_normed[:, 1])
plt.ylabel("Feature #1")
plt.xlabel("Feature #0")
plt.title("WT data")
plt.xlim([-1,1])
...
plt.figure(figsize=(12, 8))
plt.scatter(rbf_GLU_kernel_pca_normed[:, 0], rbf_GLU_kernel_pca_normed[:, 1], color='r', alpha=0.5, label='K50E MUT')
plt.scatter(rbf_WT_kernel_pca_normed[:, 0], rbf_WT_kernel_pca_normed[:, 1], color='b', alpha=0.5, label='WT')
plt.scatter(rbf_ST_kernel_pca_normed[:, 0], rbf_ST_kernel_pca_normed[:, 1], marker='X',color='black', label='Starting Struct')
plt.xlabel('PC 1')
plt.ylabel('PC 2')
plt.legend()
plt.title('PCA Transformed Data')
plt.savefig('WTandGLUData_more_data.png', bbox_inches = "tight", dpi= 2000)
