...
kernel_pca_cosine = KernelPCA(
    n_components=None, kernel="cosine", fit_inverse_transform=True, alpha=0.1, gamma=10
)
# cosine_kernel_pca = kernel_pca_cosine.fit(train_data).transform(test_data)

cosine_kernel_pca = kernel_pca_cosine.fit(train_data)
cosine_train_kernel_pca = cosine_kernel_pca.transform(train_data)
...
kernel_pca_proj_ax.scatter(cosine_train_kernel_pca[:, 0], cosine_train_kernel_pca[:, 1])
...
cosine_GLU_kernel_pca_normed = cosine_kernel_pca.transform(GLU)
cosine_WT_kernel_pca_normed = cosine_kernel_pca.transform(WT)
cosine_ST_kernel_pca_normed = cosine_kernel_pca.transform(ST)

plt.figure(figsize=(12, 8))
plt.scatter(cosine_GLU_kernel_pca_normed[:, 0], cosine_GLU_kernel_pca_normed[:, 1])
...
plt.figure(figsize=(12, 8))
plt.scatter(cosine_WT_kernel_pca_normed[:, 0], cosine_WT_kernel_pca_normed[:, 1])
plt.ylabel("Feature #1")
plt.xlabel("Feature #0")
plt.title("WT data")
plt.xlim([-1,1])
...
plt.figure(figsize=(12, 8))
plt.scatter(cosine_GLU_kernel_pca_normed[:, 0], cosine_GLU_kernel_pca_normed[:, 1], color='r', alpha=0.5, label='K50E MUT')
plt.scatter(cosine_WT_kernel_pca_normed[:, 0], cosine_WT_kernel_pca_normed[:, 1], color='b', alpha=0.5, label='WT')
plt.scatter(cosine_ST_kernel_pca_normed[:, 0], cosine_ST_kernel_pca_normed[:, 1], marker='X',color='black', label='Starting Struct')
plt.xlabel('PC 1')
plt.ylabel('PC 2')
plt.legend()
