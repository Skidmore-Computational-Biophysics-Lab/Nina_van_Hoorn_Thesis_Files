...
kernel_pca_poly = KernelPCA(
    n_components=None, kernel="poly", fit_inverse_transform=True, alpha=0.1, gamma=10
)
# poly_kernel_pca = kernel_pca_poly.fit(train_data).transform(test_data)

poly_kernel_pca = kernel_pca_poly.fit(train_data)
poly_train_kernel_pca = poly_kernel_pca.transform(train_data)
...
kernel_pca_proj_ax.scatter(poly_train_kernel_pca[:, 0], poly_train_kernel_pca[:, 1])
...
poly_GLU_kernel_pca_normed = poly_kernel_pca.transform(GLU)
poly_WT_kernel_pca_normed = poly_kernel_pca.transform(WT)
poly_ST_kernel_pca_normed = poly_kernel_pca.transform(ST)

plt.figure(figsize=(12, 8))
plt.scatter(poly_GLU_kernel_pca_normed[:, 0], poly_GLU_kernel_pca_normed[:, 1])
...
plt.figure(figsize=(12, 8))
plt.scatter(poly_WT_kernel_pca_normed[:, 0], poly_WT_kernel_pca_normed[:, 1])
plt.ylabel("Feature #1")
plt.xlabel("Feature #0")
plt.title("WT data")
plt.xlim([-1,1])
...
plt.figure(figsize=(12, 8))
plt.scatter(poly_GLU_kernel_pca_normed[:, 0], poly_GLU_kernel_pca_normed[:, 1], color='r', alpha=0.5, label='K50E MUT')
plt.scatter(poly_WT_kernel_pca_normed[:, 0], poly_WT_kernel_pca_normed[:, 1], color='b', alpha=0.5, label='WT')
plt.scatter(poly_ST_kernel_pca_normed[:, 0], poly_ST_kernel_pca_normed[:, 1], marker='X',color='black', label='Starting Struct')
plt.xlabel('PC 1')
plt.ylabel('PC 2')
plt.legend()
