...
kernel_pca_precomputed = KernelPCA(
    n_components=None, kernel="precomputed", fit_inverse_transform=True, alpha=0.1
)
# precomputed_kernel_pca = kernel_pca_precomputed.fit(train_data).transform(test_data)

precomputed_kernel_pca = kernel_pca_precomputed.fit(train_data)
precomputed_train_kernel_pca = precomputed_kernel_pca.transform(train_data)
...
kernel_pca_proj_ax.scatter(precomputed_train_kernel_pca[:, 0], precomputed_train_kernel_pca[:, 1])
...
ST = scaler.transform(start_struct_data)

precomputed_GLU_kernel_pca_normed = precomputed_kernel_pca.transform(GLU)
precomputed_WT_kernel_pca_normed = precomputed_kernel_pca.transform(WT)
precomputed_ST_kernel_pca_normed = precomputed_kernel_pca.transform(ST)

plt.figure(figsize=(12, 8))
plt.scatter(precomputed_GLU_kernel_pca_normed[:, 0], precomputed_GLU_kernel_pca_normed[:, 1])
...
plt.figure(figsize=(12, 8))
plt.scatter(precomputed_WT_kernel_pca_normed[:, 0], precomputed_WT_kernel_pca_normed[:, 1])
plt.ylabel("Feature #1")
plt.xlabel("Feature #0")
plt.title("WT data")
plt.xlim([-1,1])
...
plt.figure(figsize=(12, 8))
plt.scatter(precomputed_GLU_kernel_pca_normed[:, 0], precomputed_GLU_kernel_pca_normed[:, 1], color='r', alpha=0.5, label='K50E MUT')
plt.scatter(precomputed_WT_kernel_pca_normed[:, 0], precomputed_WT_kernel_pca_normed[:, 1], color='b', alpha=0.5, label='WT')
plt.scatter(precomputed_ST_kernel_pca_normed[:, 0], precomputed_ST_kernel_pca_normed[:, 1], marker='X',color='black', label='Starting Struct')
plt.xlabel('PC 1')
plt.ylabel('PC 2')
plt.legend()
