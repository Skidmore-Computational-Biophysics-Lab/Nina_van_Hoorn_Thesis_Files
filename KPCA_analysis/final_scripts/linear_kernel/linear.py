...
kernel_pca_linear = KernelPCA(
    n_components=None, kernel="linear", fit_inverse_transform=True, alpha=0.1
)
# linear_kernel_pca = kernel_pca_linear.fit(train_data).transform(test_data)

linear_kernel_pca = kernel_pca_linear.fit(train_data)
linear_train_kernel_pca = linear_kernel_pca.transform(train_data)
...
kernel_pca_proj_ax.scatter(linear_train_kernel_pca[:, 0], linear_train_kernel_pca[:, 1])
...
linear_GLU_kernel_pca_normed = linear_kernel_pca.transform(GLU)
linear_WT_kernel_pca_normed = linear_kernel_pca.transform(WT)
linear_ST_kernel_pca_normed = linear_kernel_pca.transform(ST)

plt.figure(figsize=(12, 8))
plt.scatter(linear_GLU_kernel_pca_normed[:, 0], linear_GLU_kernel_pca_normed[:, 1])
...
plt.figure(figsize=(12, 8))
plt.scatter(linear_WT_kernel_pca_normed[:, 0], linear_WT_kernel_pca_normed[:, 1])
plt.ylabel("Feature #1")
plt.xlabel("Feature #0")
plt.title("WT data")
plt.xlim([-1,1])
...
plt.figure(figsize=(12, 8))
plt.scatter(linear_GLU_kernel_pca_normed[:, 0], linear_GLU_kernel_pca_normed[:, 1], color='r', alpha=0.5, label='K50E MUT')
plt.scatter(linear_WT_kernel_pca_normed[:, 0], linear_WT_kernel_pca_normed[:, 1], color='b', alpha=0.5, label='WT')
plt.scatter(linear_ST_kernel_pca_normed[:, 0], linear_ST_kernel_pca_normed[:, 1], marker='X',color='black', label='Starting Struct')
plt.xlabel('PC 1')
plt.ylabel('PC 2')
plt.legend()
